...
import { activityAmount, activityRowText, dayHeading, formatAmount, groupByDay } from "./activity-format.ts";
...
test("across every agent a row leads with who, on one agent's screen with what happened", () => {
  ...
  assert.deepEqual(
    ...
    { title: "Maze runner", detail: "Funded a purchase · 19:00", titleIsAddress: false },
  );
  assert.deepEqual(
    activityRowText(item, { name: null, withAgent: false, underDayHeading: false }, now),
    { title: "Funded a purchase", detail: "Today 19:00", titleIsAddress: false },
  );
