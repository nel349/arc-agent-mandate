...
import {SIG_VALIDATION_PASSED, SIG_VALIDATION_FAILED} from "../vendor/Constants.sol";
...
abstract contract SessionKeyPermissions is ISessionKeyPlugin, SessionKeyPermissionsLoupe {
    ...
    function _checkUserOpPermissions(PackedUserOperation calldata userOp, Call[] memory calls, address sessionKey)
        internal
        returns (uint256)
    {
        ...
        if (sessionKeyData.hasGasLimit) {
            ...
            // PORTED to EntryPoint v0.7, which is the update the original comment here asked for.
            //
            // v0.6 had no separate paymaster gas limits, so the plugin approximated the paymaster's
            // share by multiplying `verificationGasLimit` by 3 -- once for account validation, once
            // for paymaster validation, and twice more for `postOp`. v0.7 carries
            // `paymasterVerificationGasLimit` and `paymasterPostOpGasLimit` explicitly, so the
            // estimate is replaced by the real figure rather than re-tuned.
            //
            // This mirrors `EntryPoint._getRequiredPrefund` exactly, so the session key meters the
            // most the account can actually be charged -- no more, and never less.
            //
            // The paymaster limits live at fixed offsets inside `paymasterAndData`, and reading
            // them from a shorter field reverts, so they are only read when a paymaster is present.
            uint256 requiredGas = userOp.unpackVerificationGasLimit() + userOp.unpackCallGasLimit()
                + userOp.preVerificationGas;
            if (userOp.paymasterAndData.length > 0) {
                requiredGas += userOp.unpackPaymasterVerificationGasLimit() + userOp.unpackPostOpGasLimit();
            }
            uint256 maxGasFee = requiredGas * userOp.unpackMaxFeePerGas();
