...
// PORTED. Alchemy built against OpenZeppelin 4, where `toEthSignedMessageHash` lived on
// ECDSA itself. Circle pins OpenZeppelin 5.0.2, which moved it to MessageHashUtils.
import {MessageHashUtils} from "@openzeppelin/contracts/utils/cryptography/MessageHashUtils.sol";

import {CastLib} from "./vendor/CastLib.sol";
...
import {Call, IStandardExecutor} from "@circle/msca/6900/v0.7/interfaces/IStandardExecutor.sol";
import {
    AssociatedLinkedListSet, AssociatedLinkedListSetLib
} from "./vendor/AssociatedLinkedListSetLib.sol";
import {
    SetValue, SENTINEL_VALUE, SIG_VALIDATION_PASSED, SIG_VALIDATION_FAILED
} from "./vendor/Constants.sol";
...
contract SessionKeyPlugin is ISessionKeyPlugin, SessionKeyPermissions, BasePlugin {
    ...
    // PORTED. Upstream declares two dependencies, a runtime one and a user-op one, both pointing
    // at the owner plugin. On a Circle passkey MSCA the runtime slot cannot work and cannot even
    // be installed, for two independent reasons:
    //
    //   1. `WeightedWebauthnMultisigPlugin` implements no `runtimeValidationFunction` -- it
    //      inherits BasePlugin's reverting default -- so runtime validation would fail anyway.
    //      That is coherent: a passkey owner cannot sign a plain transaction, so there is no
    //      direct-call path for it to authorise.
    //   2. `PluginManager` appends each dependency to a *set*. The multisig's `FunctionId` enum
    //      has exactly one member, so both slots would be the identical reference
    //      `{multisig, USER_OP_VALIDATION_OWNER}` and the second append reverts
    //      `ItemAlreadyExists()`.
    //
    // So the runtime dependency is removed rather than pointed somewhere convenient, and the
    // management functions are left with no runtime validation function at all. That is a
    // deny, not a gap: `BaseMSCA._processPreRuntimeHooksAndValidation` reverts
    // `InvalidValidationFunctionId` on an empty reference. Managing a mandate therefore requires
    // a user operation, which is the only thing a passkey can authorise.
    ...
    function pluginManifest() external pure override returns (PluginManifest memory) {
        ...
        // PORTED. Deliberately left unset -- see the note on the dependency constants above.
        // An empty runtime validation function denies the call, so mandate management is
        // reachable only through a user operation, which is what a passkey can authorise.
        // Upstream pointed these four at the runtime dependency that no longer exists.
    ...
    /// @inheritdoc BasePlugin
    /// @dev PORTED. Alchemy's BasePlugin splits this into a public `onInstall` that delegates to
    /// an internal `_onInstall`; Circle's exposes `onInstall` directly and has no internal half,
    /// so the body moves up rather than being wrapped. Behaviour is unchanged -- the same
    /// `isNotInitialized` guard, from Circle's BasePlugin, still runs first.
